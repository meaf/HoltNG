...
public class WeatherAggregatorTest {
  ...
  @Test
  public void aggregateOneDayCsv(){

    List<WeatherStateData> inList = CSVRowParserTest.parse(getTestDataCSV());
    List<WeatherStateData> outList = WeatherAggregator.hourlyToDaily(inList, WeatherAggregator.EDataSource.csv);

    Assert.assertEquals(1, outList.size());
  }

  @Test
  public void aggregateOneDayHttp(){
//
//    List<String> strings = getTestDataCSV();
//    List<WeatherStateData> inList = strings.stream().map(CSVRowParserTest::parse).collect(Collectors.toList());
//    List<WeatherStateData> outList = WeatherAggregator.hourlyToDaily(inList, WeatherAggregator.EDataSource.http);
//
//    Assert.assertEquals(1, outList.size());
  }

  private String getTestDataCSV() {
    return "2007-01-01T02:00:00Z,2007-01-01T01:00:00Z,PT60M,0.2,0,0,0,0,271,7.4\n"
